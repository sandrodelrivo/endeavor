...
| 09 | oil reservoir | immersivepetroleum | `endeavour:extreme_cold` + `endeavour:extreme_hot` | **Caveat:** IP's `reservoir.json` has an empty placement array — IP places reservoirs through its own system, not vanilla feature placement. The biome modifier here is defensive; **IP biome restriction probably needs a separate edit to `config/immersivepetroleum/reservoirs.json` or equivalent**. Verify in-game whether reservoirs respect this. |
| 99 | uranium | immersiveengineering | (disabled) | Per design: only Create Nuclear is the atomic path; IE uranium does not spawn. Remove only, no re-add. |
...
- The four mod jars whose feature IDs we depend on: `create-1.21.1-6.0.10.jar`, `createnuclear-1.3.2-beta.3-neoforge.jar`, `create-new-age-1.1.7c+neoforge-mc1.21.1.jar`, `ImmersivePetroleum-1.21.1-4.4.1-37.jar`
